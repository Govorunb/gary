...
1. System prompt
    - System prompt should be placed at the beginning of the context. It's currently included in LLM calls, but it should just be visible to the user as a regular context message.
    ...
    - An editor dialog for the system prompt would be nice as well.
2. JSON Context
...
3. Context trimming
    ...
    - We're calling remote APIs (and OpenRouter may route the user's request to virtually any model) so we may not know a precise token limit. This means we also can't have a tokenizer, so we'll have to rely on token usage statistics returned from successful LLM API responses.
